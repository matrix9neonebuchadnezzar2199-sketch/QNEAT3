...
def help_network_prepare_links():
    return (
        "<b>概要</b><br>"
        "手描き・架空の道路を既存ネットワークに接続する<b>前処理</b>を行い、"
        "ルーティング用のネットワークレイヤを出力します。"
        "QGIS のネットワーク解析はリンクの<b>端点同士が一致する場所にしか接続を作らない</b>ため、"
        "道路の途中に突き当たる新規リンクはそのままでは経路に使われません。"
        "このツールで接続を作ってから最短経路・OD 行列・等時圏を実行してください。"
        "<br><br><b>処理内容</b><ul>"
        "<li>他リンクの<b>途中</b>に近い端点 → そのリンクを射影点で<b>分割</b>し、端点を吸着</li>"
        "<li>他リンクの<b>端点</b>に近い端点 → 座標を完全一致（スナップ）</li>"
        "<li>未入力・不正な <code>link_len</code> → 任意で実測長（楕円体）で補完</li>"
        "<li>分割したリンクの <code>link_len</code> → 実測長の比率で按分（合計は保存）</li>"
        "<li>連結成分を報告し、主成分に属さない孤立リンクを警告</li>"
        "</ul>"
        "<b>やらないこと</b>: 道路同士が交差する「途中×途中」の分割は行いません"
        "（立体交差＝非接続の意味を保持）。接続したい場所は必ず線の端点で突き当ててください。"
        "<br><br><b>使い方</b><ol>"
        "<li>既存ネットワークと架空道路を 1 つの線レイヤにまとめる</li>"
        "<li>このツールを実行（接続スナップ許容差はレイヤの CRS 単位。投影 CRS 推奨）</li>"
        "<li>出力レイヤを最短経路・OD 行列・等時圏のネットワーク入力に指定</li>"
        "</ol>"
        "経路が架空道路を通るかは<b>最小コスト選択</b>で決まります。"
        "通したい道路は <code>link_len</code> を短めに（時間モードなら速度を高めに）設定してください。"
    )
