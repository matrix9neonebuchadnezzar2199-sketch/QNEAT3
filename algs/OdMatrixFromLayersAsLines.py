...
from QNEAT3.Qneat3Utilities import (
    getFieldDatatype,
    iter_point_features,
    reconstruct_shortest_path_polyline,
)

from QNEAT3.Qneat3Strings import UIS, LOG, ja, NEO_PREFIX, log_msg
from QNEAT3.Qneat3HelpJa import help_od_matrix_layers_lines
...
class OdMatrixFromLayersAsLines(QgisAlgorithm):
    ...
    def displayName(self):
        return ja(NEO_PREFIX + 'OD 行列（レイヤ間・ライン m:n）')

    def shortHelpString(self):
        return help_od_matrix_layers_lines()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        list_from_apoints = [
            Qneat3AnalysisPoint(
                "from", feature, from_id_field, net, net.list_tiedPoints[i],
                entry_cost_calc_method, feedback, point_geom=pt,
            )
            for i, (feature, pt) in enumerate(from_rows)
        ]
        list_to_apoints = [
            Qneat3AnalysisPoint(
            ...
            for i, (feature, pt) in enumerate(to_rows)
        ]
