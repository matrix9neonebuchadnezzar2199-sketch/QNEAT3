...
from QNEAT3.Qneat3Utilities import getFieldDatatype, iter_point_features, reconstruct_path_geometry, log_far_tie_summary, reproject_rows_to_crs
...
class OdMatrixFromPointsAsLines(QgisAlgorithm):
    ...
    def displayName(self):
        return ja(NEO_PREFIX + 'OD 行列（ポイント・ライン n:n）')

    def shortHelpString(self):
        return help_od_matrix_points_lines()
    ...
    def initAlgorithm(self, config=None):
        ...
        self.MATRIX_GEOMETRY_TYPES = [ja(UIS.MATRIX_GEOM_STRAIGHT),
                                    ja(UIS.MATRIX_GEOM_ROUTE)
                                    ]
        ...
        self.addParameter(QgsProcessingParameterEnum(
            self.MATRIX_GEOMETRY_TYPE,
            ja(UIS.MATRIX_GEOM_STYLE),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # マージして渡すため Framework の変換が効かない。レイヤの CRS から変換
        point_rows = reproject_rows_to_crs(
            list(iter_point_features(points)),
            points.sourceCrs(), analysisCrs, feedback,
        )
        merged_coords = [pt for _, pt in point_rows]

        log_msg(feedback, LOG.ALG_BUILD_GRAPH)
        net = Qneat3Network(
            network, merged_coords, strategy, directionFieldName, forwardValue,
            backwardValue, bothValue, defaultDirection, analysisCrs, speedFieldName,
            defaultSpeed, tolerance, link_length_field, feedback,
        )

        list_analysis_points = [
            Qneat3AnalysisPoint(
                "point", feature, id_field, net, net.list_tiedPoints[i],
                entry_cost_calc_method, feedback, point_geom=pt,
            )
            for i, (feature, pt) in enumerate(point_rows)
        ]
