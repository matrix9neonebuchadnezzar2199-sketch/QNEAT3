...
class Qneat3Network():
    ...
    def __init__(self, 
                 ...
                 input_link_length_field, #str — 距離最適化時必須（例: link_len）
        ...
        push_build_banner(self.feedback)

        log_msg(self.feedback, LOG.NET_SETUP)
        ...
        log_msg(self.feedback, LOG.NET_TIE_START)
        log_msg(self.feedback, LOG.NET_TIE_HEAVY)
        start_local_time = time.localtime()
        start_time = time.time()
        log_msg(
            self.feedback,
            LOG.NET_START_TIME,
            time=time.strftime("%Y-%m-%d %H:%M:%S", start_local_time),
        )
        log_msg(self.feedback, LOG.NET_BUILDING)
    ...
    def setNetworkStrategy(
        ...
        self._link_len_field_name = input_link_length_field

        if input_strategy == 0:
            self.strategy_int = 0
            self.strategy = Qneat3LinkLengthStrategy(
                link_field_index, input_link_length_field, self.AnalysisCrs
            )
        else:
    ...
    def calcIsoPoints(self, analysis_point_list, max_dist):
        ...
        for counter, point in enumerate(analysis_point_list):
            log_msg(self.feedback, LOG.ISO_PROCESS_POINT, counter=counter)
                ...
                if tree[i] != -1:
                    ...
                    if real_cost <= max_dist:
                        ...
                        if fromVertexId not in iso_pointcloud:
                            iso_pointcloud[fromVertexId] = feat
                        elif iso_pointcloud[fromVertexId]['cost'] > real_cost:
                            iso_pointcloud[fromVertexId] = feat
                        #count up to next vertex
                i = i + 1 
                if (i%10000)==0:
                    log_msg(self.feedback, LOG.ISO_NODES_ADDED, n=i)
    ...
    def calcQneatInterpolation(self,iso_pointcloud_featurelist, resolution, interpolation_raster_path):  
        ...
        log_msg(self.feedback, LOG.INTERP_BEGIN)
        total_work = rows * cols
        counter = 0
        
        log_msg(self.feedback, LOG.INTERP_WORKLOAD, n=total_work)
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                counter = counter+1
                if counter%1000 == 0:
                    log_msg(self.feedback, LOG.INTERP_PROGRESS, n=counter)
    ...
    def calcIsoTinInterpolation(self, iso_point_layer, resolution, interpolation_raster_path):
        if self.AnalysisCrs.isGeographic():
            raise QgsProcessingException(ERR.TIN_GEOGRAPHIC)
    ...
    def calcIsoContours(self, max_dist, interval, interpolation_raster_path):
        ...
        for current_level in nditer(levels):
            log_msg(self.feedback, LOG.ISO_CONTOUR_LEVEL, level=current_level)
    ...
    def calcIsoPolygons(self, max_dist, interval, interpolation_raster_path):
        ...
        log_msg(self.feedback, LOG.ISO_POLYGON_COUNT, n=len(featurelist))
        return featurelist
